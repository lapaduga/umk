...
window.onload = function () {
	...
	if (menuItems.length > 0) {
		var _loop = function _loop(index) {
			...
			if (menuItem.querySelector('.menu__sub-list')) {
				menuItem.querySelector('.menu__link').addEventListener('click', function (e) {
					e.preventDefault();
					menuItem.classList.toggle('_active');
					closeArea.classList.toggle('_active');
					document.querySelector('.header__phone').classList.toggle('_hide');
					document.querySelector('.header__btn').classList.toggle('_hide');

					for (var _index = 0; _index < menuItems.length; _index++) {
						var element = menuItems[_index];
						element.classList.toggle('_hide');
					}

					e.currentTarget.closest('li').classList.remove('_hide');
					e.currentTarget.classList.toggle('_cross');
				});
			}

			closeArea.addEventListener('click', function () {
				menuItem.classList.remove('_active');
				closeArea.classList.remove('_active');
				document.querySelector('.header__phone').classList.remove('_hide');
				document.querySelector('.header__btn').classList.remove('_hide');
				document.body.classList.remove('_lock');

				for (var _index2 = 0; _index2 < menuItems.length; _index2++) {
					var element = menuItems[_index2];
					element.classList.remove('_hide');
					element.querySelector('.menu__link').classList.remove('_cross');
				}
			});
		};
	...
	$(function () {
		$('.mainscreen__scroll-down').on('click', function (e) {
	...
	$('.filter__item span').click(function () {
		$(this).parent().toggleClass('_active').children('.filter__sub-list').slideToggle();
	});
	$('.filter__sub-item').click(function () {
		$(this).toggleClass('_highlight');
	});
	$('.shop__filter-icon').click(function () {
		$('.filter').toggleClass('_show');
		document.body.classList.toggle('_no-events');
	});
	//#endregion
	...
	$('.tabs__tab').click(function (e) {
		...
		$('.tabs__worksheet').fadeOut(300);

		$(this).addClass('tabs__tab--active');

		$($(this).attr('href')).delay(250).fadeIn(300);
	});
	//#endregion

	//#region Maps
	$('#scheme-ser').on('click', function (e) {
		e.preventDefault();
		$('#map-che').fadeOut(300);
		$('#map-ser').delay(300).fadeIn(300);
	});
	$('#scheme-che').on('click', function (e) {
		e.preventDefault();
		$('#map-ser').fadeOut(300);
		$('#map-che').delay(300).fadeIn(300);
	});
	$(function () {
		$('.contacts__scheme').on('click', function (e) {
			$('html,body').stop().animate({ scrollTop: $(this.hash).offset().top - 100 }, 500);
		});
	});
	//#endregion
};
...
!function (t) {
  ...
  }return { __spectrum: i, version: lt, cssClasses: u, create: function create(t, e) {
